...
function accountPanel(title,body,actions){
  ...
  const el=document.createElement('div');el.className='mk-account-panel';el.innerHTML=`<div class="mk-account-box"><h3>${title}</h3><div>${body}</div><div class="mk-account-actions">${actions}</div></div>`;document.body.appendChild(el);return el;
}
...
async function toggleAccount(id,label,button){
  const current=await statusFor(id);const next=current.status==='active'?'inactive':'active';
  const panel=accountPanel(next==='inactive'?'Hesabı pasife al',`<p><b>${label}</b> hesabını pasife almak istediğine emin misin?</p><p class="mk-account-muted">${current.role==='admin'?'Bu öğretmene bağlı öğrenci ve velilerin aktif oturumları da kapatılır.':current.role==='student'?'Bu öğrencinin bağlı olduğu veli hesabının aktif oturumu da kapatılır.':''}</p>`,`<button type="button" data-cancel>Vazgeç</button><button type="button" class="primary">${next==='inactive'?'Pasife Al':'Aktifleştir'}</button>`);
  panel.querySelector('[data-cancel]').onclick=()=>panel.remove();
  panel.querySelector('.primary').onclick=async()=>{try{await accountApi('/api/account-status',{method:'POST',body:JSON.stringify({id,status:next})});panel.remove();button.dataset.status=next;button.textContent=next==='active'?'Pasife Al':'Aktifleştir';button.classList.toggle('active',next==='active');accountToast(next==='active'?'Hesap aktifleştirildi.':'Hesap pasife alındı.');}catch(e){accountToast(e.message,'error')}};
}
...
const accountObs=new MutationObserver(()=>decorateAccountButtons());accountObs.observe(document.documentElement,{subtree:true,childList:true});setTimeout(decorateAccountButtons,500);setInterval(decorateAccountButtons,1500);
