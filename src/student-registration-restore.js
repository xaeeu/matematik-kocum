(()=>{
  ...
  const open=async()=>{
    ...
    const data=await api('/api/data').catch(()=>({teachers:[]}));
    const owners=data.teachers||[];
    const ownerField=userFromStorage()?.role==='superadmin'?`<label class="field"><span>Öğretmen</span><select name="ownerId">${owners.map(t=>`<option value="${esc(t.id)}">${esc(t.name)}</option>`).join('')}</select></label>`:'';
    const modal=document.createElement('div');modal.className='modal-backdrop';modal.id='student-register-modal';modal.innerHTML=`<div class="modal-card"><div class="modal-head"><h2>Öğrenci ve Veli Ekle</h2><button class="icon-btn" data-close>×</button></div><div class="modal-body"><form id="student-register-form" class="form-grid">
    ...
    modal.querySelector('[data-close]').onclick=close;modal.querySelector('[data-cancel]').onclick=close;modal.addEventListener('click',e=>{if(e.target===modal)close()});
