(()=>{
  ...
  function styles(){
    ...
    s.textContent=`
      ...
      .rs-student-summary{display:flex;flex-wrap:wrap;gap:8px;margin:0 0 12px}.rs-chip{border:1px solid #e2e8f0;background:#f9fbfd;border-radius:999px;padding:6px 9px;font-size:10px;font-weight:800;color:#5d6d84}.rs-chip.ok{background:#eaf7ef;border-color:#cdebd8;color:#267647}
      @media(max-width:1000px){.rs-summary{grid-template-columns:1fr 1fr}.rs-quick{grid-template-columns:1fr 1fr}}
  ...
  function baseStats(){
    ...
    if(wrap.dataset.rsBase)return JSON.parse(wrap.dataset.rsBase);
    const out={};
    wrap.querySelectorAll('.stat').forEach(c=>{const l=c.querySelector('span')?.textContent?.trim();const v=c.querySelector('b')?.textContent?.trim()||'0';if(l)out[l]=v;});
    wrap.dataset.rsBase=JSON.stringify(out);return out;
  }
  function base(label){return baseStats()[label]||'0'}
  function replaceStats(items){
    const wrap=document.querySelector('.stats');if(!wrap)return;
    if(wrap.dataset.rsRendered===role())return;
    ...
    wrap.classList.add('rs-summary');wrap.dataset.rsRendered=role();
  }
  function quickItems(){
    const r=role();
    ...
    if(r==='superadmin')return [['Öğrenciler','Tüm öğrencileri incele','◉','students'],['Takvim','Öğretmen takvimlerini görüntüle','▦','calendar'],['Denemeler','Sınav kayıtlarını incele','▤','exams'],['Öğrenciler','Öğrenci durumlarını yönet','◎','students']];
  ...
  function ensureQuick(){
    ...
    const root=content();if(!root)return;const items=quickItems();if(!items.length)return;
    ...
    const hero=root.querySelector('.mk-hero')||root.querySelector('.hero');(hero||root.firstElementChild)?.after(box);
    box.querySelectorAll('[data-rs-action]').forEach(b=>b.onclick=()=>{const a=b.dataset.rsAction;if(a==='new-student'){document.querySelector('[data-action="new-student"]')?.click();return;}if(a==='new-exam'){document.querySelector('[data-action="new-exam"]')?.click();return;}if(a==='videos'){document.querySelector('.mk-video-nav')?.click();return;}clickNav(a)});
  }
  function roleCopy(){
    const root=content();if(!root)return;const r=role();
    const hero=root.querySelector('.mk-hero h2,.hero h2');const sub=root.querySelector('.mk-hero p,.hero p');
    const copy={
  ...
  function roleStats(){
    const r=role();if(!document.querySelector('.stats'))return;const b=base();
    if(r==='student')replaceStats([
      ['Yaklaşan Ders',b['Yaklaşan Ders']||'0','Bugün ve sonraki dersler'],
      ['Denemeler',b['Deneme / Kazanım']||'0','Sınav kayıtları'],
      ['Koçluk',document.querySelector('[data-page="coaching"]')?'Aktif':'—','Koçluk hizmeti'],
      ['Çalışma Planı','Hazır','Günlük hedeflerin']
    ]);
    else if(r==='parent')replaceStats([
      ['Çocuk',b['Öğrenci']||'0','Bağlı öğrenci'],
      ['Yaklaşan Ders',b['Yaklaşan Ders']||'0','Planlanan dersler'],
      ['Saat Talepleri',b['Bekleyen Talep']||'0','Bekleyen değişiklikler'],
      ['Denemeler',b['Deneme / Kazanım']||'0','Sınavlar']
    ]);
    else if(r==='admin')replaceStats([
      ['Öğrencilerim',b['Öğrenci']||'0','Sadece size bağlı öğrenciler'],
      ['Yaklaşan Ders',b['Yaklaşan Ders']||'0','Kendi takviminiz'],
      ['Bekleyen Talepler',b['Bekleyen Talep']||'0','Onay bekleyen saat talepleri'],
      ['Denemeler',b['Deneme / Kazanım']||'0','Oluşturduğunuz sınavlar']
    ]);
    else if(r==='superadmin')replaceStats([
      ['Toplam Öğrenci',b['Öğrenci']||'0','Sistemdeki öğrenciler'],
      ['Yaklaşan Ders',b['Yaklaşan Ders']||'0','Planlanan dersler'],
      ['Bekleyen Talepler',b['Bekleyen Talep']||'0','Saat değişikliği'],
      ['Denemeler',b['Deneme / Kazanım']||'0','Sistem geneli']
    ]);
  }
  function studentsEnhance(){
    if(page()!=='Öğrenciler')return;
    const root=content();if(!root||root.querySelector('.rs-student-summary'))return;
    const cards=[...root.querySelectorAll('.slm-card')];if(!cards.length)return;
    const active=cards.filter(c=>!c.classList.contains('inactive')).length;const inactive=cards.filter(c=>c.classList.contains('inactive')).length;const total=cards.length;
    const summary=document.createElement('div');summary.className='rs-student-summary';
    summary.innerHTML=`<span class="rs-chip">Toplam ${total} öğrenci</span><span class="rs-chip ok">Aktif ${active}</span><span class="rs-chip ${inactive?'':'ok'}">Dondurulmuş ${inactive}</span>`;
    const panel=root.querySelector('.panel');panel?.prepend(summary);
  }
  function decorate(){styles();if(page()==='Genel Bakış'){roleCopy();roleStats();ensureQuick();}studentsEnhance();}
  let scheduled=false;const run=()=>{if(scheduled)return;scheduled=true;requestAnimationFrame(()=>{scheduled=false;try{decorate()}catch(e){console.error(e)}})};
  new MutationObserver(run).observe(document.body,{childList:true,subtree:true});[150,500,1200].forEach(ms=>setTimeout(run,ms));run();
})();
