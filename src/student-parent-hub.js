(() => {
  ...
  const addStudent = async () => {
    if(!isTeacher()) return;
    const m=modal('Öğrenci ekle',`<div class="sph-form">
      <label>Öğrenci adı soyadı<input name="studentName"></label>
      <label>Öğrenci kullanıcı adı<input name="studentUsername"></label>
      <label>Öğrenci şifresi<input name="studentPassword" type="password"></label>
      <label>Sınıf<input name="grade" placeholder="Örn. 8. sınıf"></label>
      <label>Grup<input name="groupName"></label>
      <label>Veli adı soyadı<input name="parentName"></label>
      <label>Veli kullanıcı adı<input name="parentUsername"></label>
      <label>Veli şifresi<input name="parentPassword" type="password"></label>
    </div>`,`<button class="sph-btn" data-cancel>Vazgeç</button><button class="sph-btn primary" data-save>Öğrenciyi ekle</button>`);
    m.querySelector('[data-cancel]').onclick=()=>m.remove();
    m.querySelector('[data-save]').onclick=async()=>{
      const q=n=>m.querySelector(`[name="${n}"]`).value.trim();
      if(!q('studentName')||!q('studentUsername')||!q('studentPassword')||!q('grade')||!q('parentName')||!q('parentUsername')||!q('parentPassword')){notify('Öğrenci ve veli bilgilerini eksiksiz doldurun.',true);return;}
      try{
        await api('/api/data',{method:'POST',body:JSON.stringify({type:'studentCreate',name:q('studentName'),username:q('studentUsername'),password:q('studentPassword'),grade:q('grade'),groupName:q('groupName'),parentName:q('parentName'),parentUsername:q('parentUsername'),parentPassword:q('parentPassword')})});
        m.remove(); notify('Öğrenci ve veli hesabı oluşturuldu.'); await openHub();
  ...
  const openHub = async () => {
    ...
    try {
      ...
      content.innerHTML=`<section class="sph-wrap"><div class="sph-toolbar"><div><h2>Öğrenci & Veli</h2><p>Öğrenci, veli ve hesap durumlarını aynı yerde yönetin.</p></div><button class="sph-btn primary" data-sph-add>+ Öğrenci Ekle</button></div><div class="sph-grid">${rows||'<div class="sph-empty">Henüz öğrenci bulunmuyor.</div>'}</div></section>`;
      ...
      content.querySelector('[data-sph-add]')?.addEventListener('click',addStudent);
