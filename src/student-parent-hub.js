(()=>{
  ...
  const toggleStatus = async (id,status,label) => {
    ...
    const title=next==='inactive'?'Hesabı dondur':'Hesabı aç';
    const action=next==='inactive'?'Hesabı dondur':'Hesabı aç';
    const question=next==='inactive'?'dondurmak':'açmak';
    const m=modal(title,`<p><b>${esc(label)}</b> hesabını ${question} istediğine emin misin?</p><p class="sph-modal-note">Bağlı veli hesabı da aynı duruma getirilecek.</p>`,`<button class="sph-btn" data-cancel>Vazgeç</button><button class="sph-btn ${next==='inactive'?'danger':'primary'}" data-ok>${action}</button>`);
    m.querySelector('[data-cancel]').onclick=()=>m.remove();
    m.querySelector('[data-ok]').onclick=async()=>{try{await api('/api/account-status',{method:'POST',body:JSON.stringify({id,status:next})});m.remove();notify(next==='inactive'?'Hesap donduruldu.':'Hesap açıldı.');await openHub();}catch(e){notify(e.message,true)}};
  };
  const openHub = async () => {
    if (!isTeacher()) return;
    ...
    try {
      ...
      const rows=students.map(s=>{const p=parents.find(x=>x.id===s.parentUserId);return `<article class="sph-card">
        ...
        <div class="sph-actions"><span class="sph-pill ${s.status==='inactive'?'off':'on'}">${s.status==='inactive'?'Pasif':'Aktif'}</span><button class="sph-btn ${s.status==='inactive'?'primary':'danger'}" data-toggle-id="${esc(s.userId||s.id)}" data-toggle-status="${esc(s.status||'active')}" data-toggle-label="${esc(s.name)}">${s.status==='inactive'?'Hesabı aç':'Hesabı dondur'}</button></div>
      </article>`}).join('');
