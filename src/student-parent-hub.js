(() => {
  ...
  const openHub = async () => {
    ...
    const rows = students.map(s => {
      ...
      return `<article class="sph-card"><div class="sph-student"><b>${esc(s.name)}</b><span>${esc(s.grade || 'Sınıf bilgisi yok')} · ${esc(s.username || '')}</span></div><div class="sph-parent">${p ? `<b>${esc(p.name)}</b><span>${esc(p.username || '')}</span>` : '<span class="sph-no-parent">Veli bilgisi bulunmuyor</span>'}</div><div class="sph-status"><span class="sph-pill ${s.status==='inactive'?'off':'on'}">Öğrenci: ${s.status==='inactive'?'Pasif':'Aktif'}</span>${p ? `<span class="sph-pill ${p.status==='inactive'?'off':'on'}">Veli: ${p.status==='inactive'?'Pasif':'Aktif'}</span>`:''}</div></article>`;
    }).join('');
    content.innerHTML = `<section class="sph-wrap"><div class="sph-head"><div><h2>Öğrenci & Veli</h2><p>Öğrenciler ve bağlı veli bilgileri burada birlikte görünür.</p></div></div><div class="sph-list">${rows || '<div class="sph-empty">Henüz öğrenci bulunmuyor.</div>'}</div></section>`;
